...
# -----------------------------------------------------------------------------
# Cumulative integrals in electrons 
# -----------------------------------------------------------------------------
...
# adding these for now to test other codes
band1 = (450, 650)
band2 = (730, 740)
n_pix_aperture = 300
...
def color(b1_min, b1_max, b2_min, b2_max, instr, t_exp_s, n_pix_aperture):
    snr1 = snr_for_band(b1_min, b1_max, instr, t_exp_s, n_pix_aperture)
    snr2 = snr_for_band(b2_min, b2_max, instr, t_exp_s, n_pix_aperture)
    return -2.5 * math.log10(snr1 / snr2)
...
# -----------------------------------------------------------------------------
# Quick-look plotting functions for all spectra and signal components
# -----------------------------------------------------------------------------

def plot_spectrum_sun():
    """Plot the solar photon flux spectrum."""
    Ssun_ph = f_sun(wavelength_nm)
...
def plot_spectrum_atm():
    ...
    plt.figure(figsize=(8,5))
    plt.plot(wavelength_nm, f_atm(wavelength_nm))
    plt.xlabel("Wavelength (nm)")
    plt.ylabel("Transmission (0–1)")
    plt.title("Atmospheric Transmission  f_atm(λ)")
    plt.grid(True)
    plt.tight_layout()
    plt.show(block=False); plt.pause(0.05) 

def plot_spectrum_qe():
    """Plot CCD quantum efficiency."""
    plt.figure(figsize=(8,5))
    plt.plot(wavelength_nm, f_qe(wavelength_nm))
    plt.xlabel("Wavelength (nm)")
    plt.ylabel("Electrons per photon")
    plt.title("CCD Quantum Efficiency  f_qe(λ)")
    plt.grid(True)
    plt.tight_layout()
    plt.show(block=False); plt.pause(0.05) 

def plot_spectrum_mirror():
    """Plot mirror reflectivity."""
    plt.figure(figsize=(8,5))
    plt.plot(wavelength_nm, f_mirror(wavelength_nm))
    plt.xlabel("Wavelength (nm)")
    plt.ylabel("Reflectivity (0–1)")
    plt.title("Mirror Reflectivity  f_mirror(λ)")
    plt.grid(True)
    plt.tight_layout()
    plt.show(block=False); plt.pause(0.05) 

def plot_spectrum_sky():
    """Plot the sky background photon flux."""
    Ssky_ph = sky_photon_spectrum(wavelength_nm)
    plt.figure(figsize=(8,5))
    plt.plot(wavelength_nm, Ssky_ph)
    plt.xlabel("Wavelength (nm)")
...
def plot_signal_integrand(K_scale=1.0):
    """Plot signal electrons per nm after telescope & CCD effects."""
    y_sig = signal_integrand_e_per_nm(wavelength_nm, instr, t_exp_s, K_scale)
    plt.figure(figsize=(8,5))
...
def plot_background_integrand():
    """Plot sky background electrons per nm per arcsec²."""
    y_bkg = background_integrand_e_per_nm(wavelength_nm, instr, t_exp_s)
    plt.figure(figsize=(8,5))
...
def plot_cumulative_curves(K_scale=1.0):
    """Plot cumulative signal and background electrons vs wavelength."""
    rebuild_cums(K_scale)
    plt.figure(figsize=(8,5))
    plt.plot(wavelength_nm, gCumSig_e, label="Signal (electrons)")
    plt.plot(wavelength_nm, gCumBkg_e_per_as2, label="Background (electrons / arcsec²)")
    plt.xlabel("Wavelength (nm)")
...
def plot_signal_and_background(K_scale=1.0):
    """Compare signal and background per nm on same plot."""
    sig = signal_integrand_e_per_nm(wavelength_nm, instr, t_exp_s, K_scale)
    bkg = background_integrand_e_per_nm(wavelength_nm, instr, t_exp_s)
    plt.figure(figsize=(8,5))
    plt.plot(wavelength_nm, sig, label="Signal (electrons / nm)")
    plt.plot(wavelength_nm, bkg, label="Background (electrons / nm / arcsec²)")
    plt.xlabel("Wavelength (nm)")
...
def plot_input_curves(normalize=False):
    ...
    normalize=True will scale each curve to its own maximum (so they all fit on 0–1 scale).
    """
    plt.figure(figsize=(8, 5))
    ...
    plt.plot(wavelength_nm, _norm(f_sun(wavelength_nm)), label="Sun (phot s⁻¹ m⁻² nm⁻¹)")
    plt.plot(wavelength_nm, _norm(f_atm(wavelength_nm)), label="Atmosphere (transmission)")
    plt.plot(wavelength_nm, _norm(f_mirror(wavelength_nm)), label="Optics (mirror)")
    plt.plot(wavelength_nm, _norm(f_qe(wavelength_nm)), label="CCD QE (e⁻/photon)")
    plt.plot(wavelength_nm, _norm(sky_photon_spectrum(wavelength_nm)), label="Sky (phot s⁻¹ m⁻² nm⁻¹ arcsec⁻²)")
...
def plot_type_stack_with_outliers(
    ...
    norm_nm: float = 550.0,
    sigma: float = 3.0,
    keep_threshold: float = 0.80,   # ≥80% of wavelengths kept → treat spectrum as "kept"
    show_median: bool = True,
    title_suffix: str = "",
) -> Dict[str, np.ndarray]:
    """
    Display ALL input spectra for a given taxon, highlighting those effectively removed
    by robust clipping, and overlay the robust average (and optional median).

    Returns a dict with classification info so you can reuse it:
      { 'res': <robust_average_spectra dict>,
        'frac_kept': (N_spectra,),
        'kept_idx': idx array,
        'outlier_idx': idx array }

    Notes:
      • 'keep_threshold' decides when an entire spectrum is considered "kept".
        It uses the fraction of wavelengths that passed the MAD clip.
      • Does not modify global f_ast unless you want to (you can call
        set_reflectance_from_arrays(grid_nm, res['avg']) afterwards).
    """
    # 1) choose IDs
    if ids is None:
        ids = get_ids_for_type(taxon)
        if not ids:
            raise RuntimeError(f"No IDs found for taxon '{taxon}'. Provide 'ids=' explicitly.")

    # 2) compute robust stats on your existing pipeline
    res = robust_average_spectra(ids, smass_path, grid_nm, norm_nm=norm_nm, sigma=sigma)
    X = res["stack"]                # (N_spectra, N_wave) interpolated & normalized
    keep_mask = res["mask"]         # (N_spectra, N_wave) True where within MAD envelope
    ids_kept = res["ids"]           # list of ID strings aligned with rows of X

    # 3) classify spectra by fraction of kept wavelengths
    with np.errstate(invalid="ignore"):
        frac_kept = np.nanmean(keep_mask, axis=1)  # (N_spectra,)
    ...
    # outliers first, so kept/avg sit on top
    for k in out_idx:
...
#extra plots we can print if checks are needed
# plot_spectrum_sun()
# plot_spectrum_atm()
# plot_spectrum_qe()
# plot_spectrum_mirror()
# plot_spectrum_sky()
# plot_signal_integrand()
# plot_background_integrand()
# plot_signal_and_background()
