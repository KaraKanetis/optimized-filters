...
# Always resolve to the actual data_files folder, no matter current working dir
DATA_DIR = Path(ld.__file__).resolve().parent   # -> .../optimized-filters/data_files
print("Using data dir:", DATA_DIR)              # optional sanity check

# Use the cache-aware loader (or load_all_curves if you prefer)
lam, curves = ld.get_curves(base_dir=str(DATA_DIR), processed_subdir="processed", use_cache=True)
# lam, curves = ld.load_all_curves(base_dir=str(DATA_DIR))  # <- alternative
