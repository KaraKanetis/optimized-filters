...
# read curves once
lam, curves = ld.load_all_curves(base_dir="data_files")  # folder name in your repo
...
def make_instrument(d_m, gain_e_per_ph, s_pix_m, read_noise_e, dark_e_per_pix_s=0.0):
    return {
        ...
        "read_noise": read_noise_e,   # <-- FIX: key name (no space)
...
def sky_photon_spectrum(wavelength_nm):
    ...
    # If you later load a real sky spectrum, swap in an interpolator here.
...
def signal_integrand(wavelength_nm, wavelength1_nm, wavelength2_nm, instr, t_exp_s):
    ...
    """
    wavelength = np.asarray(wavelength_nm, float)
    S_ast = asteroid_flux_raw(wavelength)     # Sun × Reflectance (real curves)
...
def color_mag(S1, dS1, S2, dS2):
    """Color (m2 - m1) and its uncertainty (paper eq. 8)."""
    eps = 1e-30
...
# If you really want 400–900 only:
...
# aperture size (spelling fixed)
n_pix_aperture = 50

# ---- Plot transmissions and QE (now using REAL curves) ----
Tatm = transmission_atm(wavelength)
Topt = transmission_optics(wavelength)
