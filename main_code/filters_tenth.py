...
if __name__ == "__main__":
    ...
    # For each type: compute robust average reflectance, normalize brightness, plot SNR heatmap
    for taxon, ids in type_sets.items():
        ...
        try:
            ...
            # representative magnitude per class (tweak if you like)
            mV_by_type = {"C": 10.0, "S": 8.5, "V": 6.5}
            K = scale_factor_from_AB_mag(mV_by_type.get(taxon, 10.0), lambda0_nm=550.0)
            rebuild_cums(K)
            # Plot using the averaged spectrum currently installed
            plot_snr_heatmap_current(f"{taxon}-avg", instr, t_exp_s, n_pix_aperture)
        except RuntimeError as e:
            print(f"[{taxon}] skipped: {e}")

# --- Also run one SNR 2-D plot for a C, S, and V *single object* (as before) ---
